...
#edited to only render piano and not incoming notes
...
def create_video(input_midi: str,
    ...
    frames_folder = os.path.join(Path.cwd(), "././data/processed/frames", midi_save_name)
        ...
        for file in files:
            if file.endswith('.mid'):
                if file not in names:
                    names[file] = 1
                    create_video(os.path.join(root, file))
                else:
                    print(f"Filename already taken! -> {file}")
